...
describe("build_epic_jql", () => {
  it("builds JQL for a single epic key", async () => {
    ...
    expect(result.jql).toBe(
      '(key in (PROJ-100) OR (parent in (PROJ-100) AND status != Closed AND issuetype != "Bug")) ORDER BY issuetype ASC, status ASC'
    );
  });
  ...
  it("builds JQL for multiple epic keys", async () => {
    ...
    expect(result.jql).toBe(
      '(key in (PROJ-100, PROJ-200) OR (parent in (PROJ-100, PROJ-200) AND status != Closed AND issuetype != "Bug")) ORDER BY issuetype ASC, status ASC'
    );
  });
  ...
  it("applies assignee filter to children only, not the epic itself", async () => {
    ...
    expect(result.jql).toBe(
      'key in (PROJ-100) OR (parent in (PROJ-100) AND status != Closed AND issuetype != "Bug" AND assignee = "712020:00000000-0000-0000-0000-000000000001") ORDER BY issuetype ASC, status ASC'
    );
  });
  ...
  it("applies statusCategories to children only, not the epic itself", async () => {
    ...
    expect(result.jql).toBe(
      'key in (PROJ-100) OR (parent in (PROJ-100) AND status != Closed AND issuetype != "Bug" AND statusCategory in ("In Progress")) ORDER BY issuetype ASC, status ASC'
    );
  });
  ...
  it("supports multiple statusCategories on children", async () => {
    ...
    expect(result.jql).toBe(
      'key in (PROJ-100) OR (parent in (PROJ-100) AND status != Closed AND issuetype != "Bug" AND statusCategory in ("In Progress", "To Do")) ORDER BY issuetype ASC, status ASC'
    );
  });
  ...
  it("combines assignee and statusCategories on children", async () => {
    ...
    expect(result.jql).toBe(
      'key in (PROJ-100) OR (parent in (PROJ-100) AND status != Closed AND issuetype != "Bug" AND assignee = "712020:00000000-0000-0000-0000-000000000001" AND statusCategory in ("To Do")) ORDER BY issuetype ASC, status ASC'
    );
  });
  ...
  it("always excludes Closed children", async () => {
    ...
    expect(result.jql).toContain('(parent in (PROJ-100) AND status != Closed AND issuetype != "Bug")');
  });

  it("always excludes Bug children", async () => {
    const result = await execute({ epicKeys: ["PROJ-100"] }) as { jql: string };
